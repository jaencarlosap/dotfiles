...
turn. Do **not** call `mcp.sh`, do not ask "shall I create it?" twice, do not
...
**If `mcp.sh tools atlassian` says auth is required**, say exactly that
(`mcporter auth atlassian`, once) and leave the file — the user can create it
from the file. Do not pretend it was created, and do not invent a key.
