...
- Change `foo`; never write `foo_v2` / `foo_new` / `foo.bak`. Git is the backup.

## 3. Files you must NOT create unless the user explicitly asked

Summaries, reports, notes, docs, demo/sample/scratch files, config duplicating
existing config, new directories for what fits in an existing one.
...
**Never delete code you were not asked to delete.** Adding `Foo()` does not
mean replacing `Bar()`: every symbol that existed before your edit must exist
after it. This really happened here — asked to "add a `Len()` method", the
agent silently deleted `Set()`, and `go build` still passed because nothing
referenced it yet. A green build does not prove you destroyed nothing.

## 7b. Never write your reasoning into the code

The code you emit is the FINAL ANSWER, not your scratch pad. Forbidden in any
file you write:

- Commented-out implementations of the very code you are writing.
- Narration of your own process: `// Done.`, `// Actually implement now:`,
  `// We could also use X`, `// Implementation below:`, `// Note: this is a
  simple implementation`.
- Repeating in a comment what the line under it plainly says.

Real example of what NOT to do — 23 lines of this were emitted above a 3-line
method that was already correct:

```go
// We lock and unlock around accessing the map length
// No need for defer since we only read
// But we still use Lock/Unlock to avoid race conditions
...
Think before you write. Then write only the code. A comment earns its place
only by explaining **why** something non-obvious is done — never **what**.
...
- [ ] `git status --short` contains nothing you cannot justify — delete stray
      files now.
- [ ] **The diff deletes nothing you were not asked to delete.** On an "add"
      or "fix" task this must come back empty:
      `git diff -U0 | grep '^-' | grep -E '(func|def|class|export|fn) '`
- [ ] No commented-out code and no narration of your own reasoning (§7b).
